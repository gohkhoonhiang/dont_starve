...
class HamletShop
  ...
  def convert_html_to_csv(input, output)
    html_to_csv(input, output) do |doc, csv|
      ...
      doc.xpath('//table/tr')[1..-1].each_with_index do |outer_row, outer_index|
        shop_name = outer_row.xpath('td')[0].text.strip
        ...
        table_row.xpath('table/tbody/tr').each do |data_row|
          ...
          data_row.xpath('td')[1..-1].each_with_index do |cell, index|
            tarray << cell.text.strip
          end
          csv << tarray
        end
